...
iteration_num = 300;   % the iteration number for expanding the tree
...
for iter = 1:iteration_num
    ...
    Near_points = [];
    temp_min_dist = xL*1000; % this distance is for finding the optimal 
    for i2=1:size(T.v,2)
     temp_distance = sqrt((x_new(1)-T.v(i2).x)^2 + (x_new(2)-T.v(i2).y)^2);
       if (temp_distance<query_range)
           x_temp = [T.v(i2).x, T.v(i2).y];
           dp_distance = temp_distance + T.v(i2).dist;
           
           if ~collisionChecking(x_temp,x_new,Imp) 
              continue
           end
           
           % find the point with minimum distance and collision free
           if ((dp_distance < temp_min_dist))
               temp_min_dist = dp_distance ;
               parent_index = i2;
               parent_point = x_temp;
           end
       end     % end of the if for finding neighbor points
    end     % end of the for loop for findin the points within certain range
    ...
    plot([parent_point(1), x_new(1)],[parent_point(2), x_new(2)],'b');
